...
use crate::{controllers::structs::ResponseAuthOk, crypt::{password, token}, managers::{tokendb, userdb}};
...
pub async fn register_user(State(pool): State<MySqlPool>, Json(userdata): Json<RegUser>) -> Result<impl IntoResponse, (StatusCode, String)> {
    if userdata.username.is_empty() && userdata.email.is_empty() && userdata.password.is_empty() { // Can't be too careful
    ...
    if let Err(why) = userdb::create_user(&pool, &userdata.username, &userdata.email, &encrypted_password).await {
        println!("Error {}", why);
        return Err((StatusCode::BAD_REQUEST, "Could not register a user".to_owned()))
    }

    let user_id: i32 = userdb::get_user_id(&pool, &userdata.username).await.unwrap(); // User id should be there like 100% percent (kinda)
    let token = token::make_token(user_id);

    tokendb::insert_token(&pool, user_id, &token).await.expect("Token should've been inserted");

    Ok(Json(ResponseAuthOk::new(token)))
}

pub async fn login_user(State(pool): State<MySqlPool>, Json(userdata): Json<LoginUser>) -> Result<impl IntoResponse, (StatusCode, String)> {
    println!("here");
    if userdata.email.is_empty() && userdata.password.is_empty() { // Can't be too careful
        return Err((StatusCode::BAD_REQUEST, "One of the fields is empty".to_owned()))
    }
